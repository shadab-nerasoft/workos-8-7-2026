...
interface ProjectCalendarTabProps {
  ...
  tasks: Task[];
  users: User[];
  /** Called when a task is clicked. The page owns the edit modal. */
  onTaskSelect?: (task: Task) => void;
}

/** Pure presentational month calendar. Data and modal handling come from the page. */
export function ProjectCalendarTab({ tasks, users, onTaskSelect }: ProjectCalendarTabProps) {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <ArrowLeft2 color="#2563eb" size={18} />
          </button>
          <button
            onClick={handleNextMonth}
            className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50"
          >
            <ArrowRight2 color="#2563eb" size={18} />
          </button>
        </div>
      </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span
                    className={`flex h-7 w-7 items-center justify-center rounded-full text-sm font-semibold ${isToday ? "bg-primary-600 text-white shadow-sm" : "text-slate-700"
                      }`}
                  >
                    {date}
                  </span>
